...
class OrderStatusChangeNotAllowedException(ServiceException):
    ...
    def __init__(self, order_id: int, status: str, comment: str | None = None):
        ...
        message = (
            f"Невозможно изменить статус заказа (id={order_id}) на '{status}'"
        )
        if comment:
            message += f": {comment}"
...
class OrderSupervisorAssignmentNotAllowedException(ServiceException):
    def __init__(self, order_id: int):
        self.order_id = order_id

        message = f"Невозможно назначить куратора для заказа (id={order_id})"
        super().__init__(message)
