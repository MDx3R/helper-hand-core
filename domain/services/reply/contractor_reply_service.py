...
from application.dtos.output import DetailedReplyOutputDTO
...
class ContractorReplyService(ABC):
    ...
    async def get_first_unapproved_reply_for_order(self, order_id: int, contractor: Contractor) -> DetailedReplyOutputDTO | None:
        ...
        Returns:
            DetailedReplyOutputDTO: DTO с подробными данными отклика. Может отсутствовать поле `order`, так как запрос выполняется относительно конкретного заказа. 
    ...
    async def get_unapproved_replies_for_order(self, order_id: int, contractor: Contractor, page: int = 1, size: int = 20) -> List[DetailedReplyOutputDTO]:
        ...
        Returns:
            List[DetailedReplyOutputDTO]: DTO с подробными данными отклика. Может отсутствовать поле `order`, так как запрос выполняется относительно конкретного заказа. 
