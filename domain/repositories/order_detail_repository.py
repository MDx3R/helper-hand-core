...
class OrderDetailRepository(ABC):
    ...
    async def create_detail(self, detail: OrderDetail) -> OrderDetail:
        """
        Создает экземпляр сведений о заказе.
        В отличии от `save_detail` в результате выполнения всегда добавляется новая запись.

        Args:
            detail (OrderDetail): экземпляр сведений о заказе. В объектах должен быть указан `order_id`.
    ...
    @abstractmethod
    async def get_available_details_by_order_id(
        self, order_id: int
    ) -> List[OrderDetail]:
        """
