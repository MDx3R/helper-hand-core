...
class TestToModelApplicationModelMapper:
    @pytest.mark.parametrize(
        "mapper, base_instance, expected_model", test_cases
    )
    def test_to_model_result_is_correct_instance(
        self,
        mapper: ApplicationModelMapper,
        base_instance: Base,
        expected_model: ApplicationModel,
    ) -> None:
...
class TestToBaseApplicationModelMapper:
    @pytest.mark.parametrize(
        "mapper, expected_base, model_instance", test_cases
    )
    def test_to_base_result_is_correct_instance(
        self,
        mapper: ApplicationModelMapper,
        model_instance: ApplicationModel,
        expected_base: Base,
    ) -> None:
...
def generate_list_test_cases() -> (
    List[Tuple[ApplicationModelMapper, List, List]]
):
...
class TestApplicationModelMapperEdgeCases:
    ...
    def test_to_model_list_with_empty_list(
        self, mapper: ApplicationModelMapper
    ) -> None:
    ...
    def test_to_model_with_missing_mapping(self) -> None:
        ...
        with pytest.raises(
            TypeError,
            match="Отсутствует соответствие между `UnmappedBase` и моделью",
        ):
    ...
    def test_to_base_with_missing_mapping(self) -> None:
        ...
        with pytest.raises(
            TypeError,
            match="Отсутствует соответствие между `UnmappedModel` и моделью SQLAlchemy",
        ):
