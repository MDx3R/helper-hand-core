...
def reset_service(use_case, notification_service):
    service = ContractorResetService(
        use_case=use_case,
...
def generate_contractor_test_cases():
    return [(t.input, t.context, t.expected) for t in generate_reset_contractor_test_cases()]

class TestUserResetService:
    @pytest.mark.asyncio
    @pytest.mark.parametrize("user_input, user_context, expected_user", generate_contractor_test_cases())
    ...
    async def test_reset_contractor_result_is_correct(
        ...
        test_case = ContractorResetTestCaseGenerator.create_different_id()
