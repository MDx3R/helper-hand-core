...
class ModelBaseFactory(ModelFactory, BaseFactory, Generic[B, M]):
    ...
    @classmethod
    def get_pair_list(
        cls, _count: int = 3, _random: bool = False, **kwargs
    ) -> List[Tuple[B, M]]:
...
class OrderFactory(ModelBaseFactory[OrderBase, Order]):
    ...
    def get_random_data(cls, **kwargs) -> Dict[str, Any]:
        return {
            ...
            "about": fk.sentence(),
            "address": fk.address(),
            "admin_id": fk.random_element(
                elements=[None, fk.random_int(min=1, max=100)]
            ),
...
class ReplyFactory(ModelBaseFactory[ReplyBase, Reply]):
    ...
    def get_random_data(cls, **kwargs) -> Dict[str, Any]:
        return {
            ...
            "paid": fk.random_element(
                elements=[None, fk.date_time_this_year()]
            ),
            "created_at": fk.date_time_this_year(),
            "updated_at": fk.date_time_this_year(),
        } | kwargs
    ...
    def _get_random_data(cls, **kwargs) -> Dict[str, Any]:
        return (
            ...
            | {
                ...
                "citizenship": fk.random_element(
                    elements=[e for e in CitizenshipEnum]
                ),
                "positions": fk.random_elements(
                    elements=[e for e in PositionEnum], length=2
                ),
            }
            | kwargs
        )
    ...
    def _get_random_data(cls, **kwargs) -> Dict[str, Any]:
        return (
            ...
            | {
                "role": RoleEnum.admin,
                "about": fk.paragraph(),
                "contractor_id": fk.random_element(
                    elements=[None, fk.random_int(min=1, max=100)]
                ),
            }
            | kwargs
